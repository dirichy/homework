...
\begin{proof}
  ...
  对于\(\mathbb{R} \)中的开集\(O \)，熟知\(A \)可表示为可数个不交开区间的并，于是由 Fatou 引理知 \(\liminf_{n}\mathbb{P}(\xi_n \in O) \geq \mathbb{P}(\xi \in O) \)。
  ...
  \begin{equation}
    \begin{aligned}
      ...
      &\overset{Fatou}{\geq} \int_{0}^{\infty} \liminf_{n} \mathbb{P}(\xi_n \in \{y:g(y)>x\}) \d x\\
...
\begin{solution}
  \begin{enumerate}
    ...
    \item 对于\(M>\norm{f}_{\infty} \)，有：
      ...
      若\(\norm{f}_{\infty}<\infty \)，则令\(M \to \norm{f}_{\infty}+ \)得 \(\limsup_{r \to \infty} \norm{f}_{r} \leq \norm{f}_{\infty} \)；
      否则\(\norm{f}_{\infty}= \infty \)，显然成立同样的不等式。
      ...
      若\(\norm{f}_{\infty}>0 \)，则令\(M \to \norm{f}_{\infty}- \)得\(\liminf_{r \to \infty} \norm{f}_{r} \geq \norm{f}_{\infty} \)；
      否则\(\norm{f}_{\infty} = 0 \)，显然成立同样的不等式。
