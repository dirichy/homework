...
\begin{solution}
    \begin{enumerate}
        \item For $g\in G$, we have $g\sum_{k=1}^n x_k=\sum_{k=1}^n gx_k$. Assume $gx_k=x_{\sigma(k)},\sigma\in S_n$, then $\sum_{k=1}^n gx_k=\sum_{k=1}^n x_k$, so $V_1$ is invariant subspace. Also, $g\sum_{k=1}^n a_k x_k=\sum_{k=1}^n a_k gx_k=\sum_{k=1}^n a_{\sigma^{-1}(k)}x_k\in V_2$. So $V_2$ is invariant subspace, too. 
        \item Since $\char K\nmid n$ we know $\sum_{k=1}^n x_k\notin V_2$, so $V_1\cap V_2=\{0\}$. Obviously $\dim V_1=1,\dim V_2=n-1$, so $V=V_1\oplus V_2$. So $\phi=\phi_{V_1}\oplus \phi_{V_2}$. 
    \end{enumerate}
...
\begin{solution}
    In \Cref{pro:1} let $n=2,K=\C,G=S_3$. Consider $\cdot:G\times \Omega\to \Omega$,
    $$\sigma\cdot x_i=\begin{cases}
        x_i,\sigma \text{ is even}\\
        x_{3-i},\sigma \text{ is odd}\\
    \end{cases}$$
    Then easily $\cdot$ is a group action. Consider $\phi:G\to \gl(V),\phi(g)(x)=g\cdot x$. We get $g(x_1+x_2)=x_1+x_2$, so $\Phi_{V_1}=I_1$. And $g(x_1-x_2)=\begin{cases}
        x_1-x_2,g\text{ is even}\\
        x_2-x_1,g\text{ is odd}
    \end{cases}$, so $\Phi_{V_2}=\pm I_1$. Finally we get the matrix representation $\Phi$:
    \begin{equation}
        \Phi(g)=\begin{cases}
            \left(
            \begin{array}{cc}
...
\begin{solution}
    \begin{enumerate}
        \item Obviously $\dim M_n(K)=n^2$, so only need to prove $\phi$ is group homomorphism. We have $\phi(AB)X=(AB)X(AB)^{-1}=A(BXB^{-1})A^{-1}=\phi(A)(\phi(B)X)=\phi(A)\phi(B)X$, so $\phi(AB)=\phi(A)\phi(B)$. 
        \item Since $\phi(A)$ is Similarity transformation over $M_n(K)$, so $\tra(\phi(A)X)=\tra X$. So $M_n^0(K)$ is invariant subspace. Noting $\phi(A)I=AIA^{-1}=I$, so $\<I\>$ is invariant subspace, too. 
        \item Obviously $\dim M_n^0(K)=n^2-1,\dim \<I\>=1$. Since $\char K\nmid n$, we get $\tra I=n\neq 0$, so $M_n^0(K)\oplus \<I\>=M_n(K)$. So $\phi=\phi_{\<I\>}\oplus \phi_{M_n^0(K)}$. 
    \end{enumerate}
\end{solution}
...
\begin{solution}
    \begin{enumerate}
        \item Since $(\phi(A)X)^T=(A^{-1})^TX^TA^T=\phi((A^{-1})^T)X^T=\phi(A)X^T$, so $M_n^+,M_n^-$ is invariant subspace. 
        \item Only need to prove $M_n(\R)=\<I\>\oplus M_n^+(\R)\oplus M_n^-(\R)$. From \Cref{pro:2} we know $M_n(\R)=\<I\>\oplus M_n^0(\R)$, so we only need to prove $M_n^0(\R)=M_n^+(\R)\oplus M_n^-(\R)$. For $A\in M_n^0(\R)$, we have $A=\frac{A+A^T}{2}+\frac{A-A^T}{2}$, where $\frac{A+A^T}{2}\in M_n^+(\R)$ and $\frac{A-A^T}{2}\in M_n^-(\R)$. So we only need to prove $M_n^+\cap M_n^-=\{0\}$. If $A\in M_n^+\cap M_n^-$, then $A=A^T=-A^T$, so $A=0$. So $M_n^0(\R)=M_n^+(\R)\oplus M_n^-(\R)$, thus $\varphi=\varphi_0\oplus\varphi_1\oplus\varphi_2$.
        \item Obviously $\dim M_n^-(\R)=\frac{1}{2}n(n-1)$, so $\phi_2$ satisfy the condition. 
    \end{enumerate}
\end{solution}
