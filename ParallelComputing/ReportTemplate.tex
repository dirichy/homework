...
\begin{equation}
  \begin{aligned}H(\sigma )&=-\sum _{ij\in E(V^{+})}J_{ij}-\sum _{ij\in E(V^{-})}J_{ij}+\sum _{ij\in \delta (V^{+})}J_{ij}\\&=-\sum _{ij\in E(G)}J_{ij}+2\sum _{ij\in \delta (V^{+})}J_{ij}
  \end{aligned}
\end{equation}
...
\section{Codes}
\newcommand{\code}[1]{
\subsection{#1}\lstinputlisting[frame=single, numbers=left, style=Matlab-editor]{codes/#1.m}}
\code{main}
\code{GenerateGrid}
\code{WhetherAccept}
\code{getDeltaEnergy}
